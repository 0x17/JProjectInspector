...
public class CoverageHeuristic {
	...
	public static List<String> listUntestedMethods(String classPath, Map<String, String> hasTestIndex) throws Exception {
		...
		List<String> methodsInProduct = JavaSourceHelpers.listMethodNamesInSrcStr(FileHelpers.readEntireFile(new File(classPath)));
		
		if(testPath != null) {
			List<String> methodsInTest = JavaSourceHelpers.listMethodNamesInSrcStr(FileHelpers.readEntireFile(new File(testPath)));
	...
	public static List<String> determineAllMethods(String rootPath) throws Exception {
		...
		for(String classPath : classPaths) {
			...
			allMethods.addAll(JavaSourceHelpers.listMethodNamesInSrcStr(srcStr));
		}
		return allMethods;
	}
